...
class ImgCompression(object):
    ...
    def svd(self, X: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:  # [4pts]
        ...
        if len(X.shape) == 2:
            ...
            for i in range(3):
                U[i], S[i], V[i] = np.linalg.svd(X[i], full_matrices=False)
    ...
    def rebuild_svd(
        ...
        if len(U_compressed.shape) == 2:
            ...
            for i in range(3):
                Xrebuild[i] = np.matmul(np.matmul(U_compressed[i], np.diag(S_compressed[i])), V_compressed[i])
    ...
    def compression_ratio(self, X: np.ndarray, k: int) -> float:  # [4pts]
        ...
        # if len(X.shape) == 2:
        #     compression_ration = (X.shape[0] * k + k + k * X.shape[1]) / (X.shape[0] * X.shape[1])
        # else:
        #     compression_ration = (X.shape[1] * k + 3 * k + 3 * k * X.shape[2]) / (3 * X.shape[1] * X.shape[2])
            
        # print(compression_ration)
        ...
        if len(X.shape) == 2:  # Black and white image
            ...
            compressed_size = 3 * X.shape[1] * k + 3 * k + 3 * k * X.shape[2]
    ...
    def recovered_variance_proportion(self, S: np.ndarray, k: int) -> float:  # [4pts]
        ...
        if len(S.shape) == 1:  # Handling black and white images
            recovered_var = np.sum(S[:k]**2) / np.sum(S**2)
        else:  # Handling color images
