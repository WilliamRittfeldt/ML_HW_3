...
class PCA(object):
    ...
    def fit(self, X: np.ndarray) -> None:  # 5 points
        ...
        #make self.U: (N, min(N,D)) numpy array 
        self.U = self.U[:, :min(X.shape[0], X.shape[1])]
        self.S = self.S[:min(X.shape[0], X.shape[1])]
        self.V = self.V[:min(X.shape[0], X.shape[1]), :]
    ...
    def transform_rv(
        ...
        # Determine the number of principal components needed to retain the specified variance
    ...
    def visualize(self, X: np.ndarray, y: np.ndarray, fig_title) -> None:  # 5 pts
        ...
        # Reduce dataset to 2 features for 2D scatter plot
        X_2d = self.transform(X, K=2)
        df_2d = pd.DataFrame(X_2d, columns=['PC1', 'PC2'])
        df_2d['Label'] = y

        # Reduce dataset to 3 features for 3D scatter plot
        X_3d = self.transform(X, K=3)
        df_3d = pd.DataFrame(X_3d, columns=['PC1', 'PC2', 'PC3'])
        df_3d['Label'] = y

        # Create 2D scatter plot
        fig_2d = px.scatter(df_2d, x='PC1', y='PC2', color='Label', title=f'{fig_title} (2D)')
        fig_2d.show()

        # Create 3D scatter plot
        fig_3d = px.scatter_3d(df_3d, x='PC1', y='PC2', z='PC3', color='Label', title=f'{fig_title} (3D)')
        fig_3d.show()
